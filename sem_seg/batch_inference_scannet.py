# adapted from batch_inference.py from PointNet
# uses the Scan-Net dataset instead, doesn't calculate IOU etc, just makes predictions
...
def evaluate():
    ...
    with tf.device('/gpu:'+str(GPU_INDEX)):
        ...
        loss = get_loss(pred, labels_pl)  #  remove, since not needed for predictions
...
def eval_one_epoch(sess, ops, room_path, out_data_label_filename):  # , out_gt_label_filename):
    ...
    # Get room dimension..
    data_label = mesh  # np.load(room_path)
    data = data_label[:,0:6]
    ...
    for batch_idx in range(num_batches):
        ...
        if FLAGS.no_clutter:
            ...
            pred_label = np.argmax(pred_val, 3) # BxN
